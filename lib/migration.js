...
function mixinMigration(PostgreSQL) {
  ...
  function normalizeIndexKeyDefinition(keys, supportsOrdering = true) {
    // normalize:
    //  {'column1': -1, 'column2': 1}
    ...
    if (typeof keys === 'string') {
      ...
      result = keys.map(function(column) {
        ...
        return column && [column, supportsOrdering ? "ASC" : ""];
      });
    } else {
  ...
  function normalizeIndexDefinition(index) {
    if (typeof index === 'object'  && index.keys) {
      ...
      index.keys = normalizeIndexKeyDefinition(index.keys, index.type !== 'gin' && index.type !== 'gist');
      return index;
    }
